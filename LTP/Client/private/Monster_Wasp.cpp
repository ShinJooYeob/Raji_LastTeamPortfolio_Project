...
void CMonster_Wasp::CollisionTriger(CCollider * pMyCollider, _uint iMyColliderIndex, CGameObject * pConflictedObj, CCollider * pConflictedCollider, _uint iConflictedObjColliderIndex, CollisionTypeID eConflictedObjCollisionType)
{
	...
	//if (CollisionTypeID::CollisionType_PlayerWeapon == eConflictedObjCollisionType && m_vecInstancedTransform[iMyColliderIndex].bHit == false)
	//{
	//	m_vecInstancedTransform[iMyColliderIndex].iRenderType = RENDER_HIT;
	//	m_vecInstancedTransform[iMyColliderIndex].iHp += -1;
	//	m_vecInstancedTransform[iMyColliderIndex].dTime = 0;
	//	m_vecInstancedTransform[iMyColliderIndex].bHit = true;

	//	m_vecInstancedTransform[iMyColliderIndex].fRimRight.w = 1;

	//	if (m_vecInstancedTransform[iMyColliderIndex].iHp <= 0)
	//	{
	//		m_vecInstancedTransform[iMyColliderIndex].iRenderType = RENDMER_DIE;
	//	}
	//}
}
...
HRESULT CMonster_Wasp::SetUp_Info()
{
	...
	for (_uint i = 0; i < ANIM_END; i++)
	{
		...
		CModelInstance* pModelInstance = (CModelInstance*)g_pGameInstance->Clone_Component(m_eNowSceneNum, TAG_CP(Prototype_ModelInstance_128), &tModelIntDsec);
...
HRESULT CMonster_Wasp::Update_Collider(_double dDeltaTime)
{
	//////////////Collider
...
HRESULT CMonster_Wasp::Update_VectorGroup(_double dDeltaTime)
{
