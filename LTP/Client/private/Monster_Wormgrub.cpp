...
HRESULT CMonster_Wormgrub::Initialize_Clone(void * pArg)
{
	...
	if (nullptr != pArg)
	{
		...
		MSGBOX("CMonster_Wormgrub의 pArg가 nullptr입니다.");
	}
	...
	for (_uint i = 0; i < m_Instance_Info.fValueMat.m[0][1]; i++)
	{
		...
		for (_uint j = (_uint)m_Instance_Info.fValueMat.m[0][3]; j < (_uint)m_Instance_Info.fValueMat.m[0][2]; i++)
		{
			...
			if ((_uint)m_Instance_Info.fSubValueMat.m[X][Y] > 0)
			{
				...
				MSGBOX("CMonster_Wormgrub의 m_Instance_Info.fSubValueMat.m[X][Y]가 0 이하입니다.");
			}
		}
		...
		AttackColliderDesc.vPosition = _float4(0.f, 0.5f, 0.7f, 1.f);
...
HRESULT CMonster_Wormgrub::Update_Render(_double dDeltaTime)
{
	return S_OK;
}

HRESULT CMonster_Wormgrub::Update_VectorGroup(_double dDeltaTime)
{
...
HRESULT CMonster_Wormgrub::Adjust_AnimMovedTransform(_double dDeltatime)
{
	for (_int i = 0; i < m_vecInstancedTransform.size(); i++)
	{
		...
		{
		case ANIM_RUN_Frame1:
			m_vecInstancedTransform[i].pTransform->Move_Forward(dDeltatime *0.9, m_vecInstancedTransform[i].pNavigation);
			break;
		case ANIM_RUN_Frame2:
			m_vecInstancedTransform[i].pTransform->Move_Forward(dDeltatime*0.9, m_vecInstancedTransform[i].pNavigation);
			break;
		default:
		{
			if (m_vecInstancedTransform[i].iSwtichIndex == 0 && m_pModel[m_vecInstancedTransform[i].iAnimType]->Get_PlayRate() >= 0.4 && m_pModel[m_vecInstancedTransform[i].iAnimType]->Get_PlayRate() <= 0.45)
			{
				m_bAttackOn = true;
			...
			else if (m_vecInstancedTransform[i].iSwtichIndex == 1 && m_pModel[m_vecInstancedTransform[i].iAnimType]->Get_PlayRate() >= 0.85)
			{
		...
		if (PlayRate <= 0.95)
		{
			if (i >= ANIM_RUN_Frame1 && i <= ANIM_RUN_Frame2)
			{
				if (m_iSoundIndex[i] == 0 && m_bSoundSwitch[i] == true && m_pModel[i]->Get_PlayRate() >= 0.1)
				{
					...
					g_pGameInstance->Play3D_Sound(TEXT("EH_M1_2243.mp3"), m_pPlayerTransformCom->Get_MatrixState(CTransform::STATE_POS), CHANNELID::CHANNEL_MONSTER, 0.1f);
					m_iSoundIndex[i]++;
				}
			}
			if (i >= ANIM_ATTACK_Frame1 && i <= ANIM_ATTACK_Frame5)
			{
				...
				{
					g_pGameInstance->Play3D_Sound(TEXT("EH_M1_470.mp3"), m_pPlayerTransformCom->Get_MatrixState(CTransform::STATE_POS), CHANNELID::CHANNEL_MONSTER, 0.2f);
					m_iSoundIndex[i]++;
				}
			}
		}
