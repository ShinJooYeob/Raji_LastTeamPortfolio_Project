...
HRESULT CMonster_Wolf::Initialize_Clone(void * pArg)
{
	...
	if (nullptr != pArg)
	{
		...
		MSGBOX("CMonster_Wolf의 pArg가 nullptr입니다.");
	}
...
void CMonster_Wolf::CollisionTriger(CCollider * pMyCollider, _uint iMyColliderIndex, CGameObject * pConflictedObj, CCollider * pConflictedCollider, _uint iConflictedObjColliderIndex, CollisionTypeID eConflictedObjCollisionType)
{
	...
	for (_uint i = 0; i < m_Instance_Info.fValueMat.m[0][1]; i++)
	{
		...
		for (_uint j = (_uint)m_Instance_Info.fValueMat.m[0][3]; j < (_uint)m_Instance_Info.fValueMat.m[0][2]; i++)
		{
			...
			if ((_uint)m_Instance_Info.fSubValueMat.m[X][Y] > 0)
			{
				...
				MSGBOX("CMonster_Wolf의 m_Instance_Info.fSubValueMat.m[X][Y]가 0 이하입니다.");
			}
		}
...
HRESULT CMonster_Wolf::Update_Collider(_double dDeltaTime)
{
	//////////////Collider
		...
		if (PlayRate <= 0.95)
		{
			for (_int j = 0; j < m_vecInstancedTransform.size(); j++)
			{
				...
				if (i >= ANIM_RUN_Frame1 && i <= ANIM_RUN_Frame2)
				{
					...
					{
						g_pGameInstance->Play3D_Sound(TEXT("EH_Tezabsura_Footstep_01.wav"), m_pPlayerTransformCom->Get_MatrixState(CTransform::STATE_POS), CHANNELID::CHANNEL_MONSTER, 0.1f);
						m_iSoundIndex[i]++;
						break;
					}
				}
