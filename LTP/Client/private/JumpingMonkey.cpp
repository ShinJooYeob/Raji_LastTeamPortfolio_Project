...
CJumpingMonkey::CJumpingMonkey(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CMiniGame_Jino_Monster(pDevice, pDeviceContext)
{
}

CJumpingMonkey::CJumpingMonkey(const CJumpingMonkey & rhs)
	: CMiniGame_Jino_Monster(rhs)
{
}

HRESULT CJumpingMonkey::Initialize_Prototype(void * pArg)
{
...
HRESULT CJumpingMonkey::Initialize_Clone(void * pArg)
{
...
_int CJumpingMonkey::LateRender()
{
	return _int();
}

void CJumpingMonkey::CollisionTriger(CCollider * pMyCollider, _uint iMyColliderIndex, CGameObject * pConflictedObj, CCollider * pConflictedCollider, _uint iConflictedObjColliderIndex, CollisionTypeID eConflictedObjCollisionType)
{
...
_float CJumpingMonkey::Take_Damage(CGameObject * pTargetObject, _float fDamageAmount, _fVector vDamageDir, _bool bKnockback, _float fKnockbackPower)
{
	return _float();
}
...
void CJumpingMonkey::Set_Active(_bool bActive)
{
	m_bActive = bActive;
}

void CJumpingMonkey::Set_State_Move()
{
	m_eCurState = ANIM_MOVE;
	m_pModel->Change_AnimIndex(1);
}

void CJumpingMonkey::Set_State_JumpStart()
{
	m_eCurState = ANIM_JUMP;
...
void CJumpingMonkey::Set_Position(_float3 vPos)
{
	m_pTransformCom->Set_MatrixState(CTransform::TransformState::STATE_POS, vPos);
}

HRESULT CJumpingMonkey::SetUp_Components()
{
...
HRESULT CJumpingMonkey::SetUp_Colliders()
{
	...
	ColliderDesc.vRotation = _float4(0.f, 0.f, 0.f, 1.f);
	ColliderDesc.vPosition = _float4(0.3f, 0.f, 0.3f, 1.f);
	FAILED_CHECK(m_pCollider->Add_ColliderBuffer(COLLIDER_SPHERE, &ColliderDesc));
	tAttachedDesc = ATTACHEDESC();
	tAttachedDesc.Initialize_AttachedDesc(this, "head", _float3(0.9f, 0.9f, 0.9f), _float3(0.f, 0.f, 0.f), _float3(0.f, 0.03226f, -2.3082f));
	m_vecAttachedDesc.push_back(tAttachedDesc);
	m_pCollider->Set_ParantBuffer();


	ZeroMemory(&ColliderDesc, sizeof(COLLIDERDESC));
	ColliderDesc.vScale = _float3(0.9f, 0.9f, 0.9f);
	ColliderDesc.vRotation = _float4(0.f, 0.f, 0.f, 1.f);
	ColliderDesc.vPosition = _float4(0.f, 0.f, 0.f, 1.f);
	FAILED_CHECK(m_pCollider->Add_ColliderBuffer(COLLIDER_SPHERE, &ColliderDesc));
	tAttachedDesc = ATTACHEDESC();
...
HRESULT CJumpingMonkey::SetUp_Etc()
{
...
void CJumpingMonkey::Update_Move(_double fDeltaTime)
{
...
HRESULT CJumpingMonkey::Update_Collider(_double fDeltaTime)
{
	//Collider
...
CJumpingMonkey * CJumpingMonkey::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext, void * pArg)
{
	CJumpingMonkey*	pInstance = NEW CJumpingMonkey(pDevice, pDeviceContext);

	if (FAILED(pInstance->Initialize_Prototype(pArg)))
	{
		MSGBOX("Failed to Created CJumpingMonkey");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CJumpingMonkey::Clone(void * pArg)
{
	CJumpingMonkey*	pInstance = NEW CJumpingMonkey(*this);
