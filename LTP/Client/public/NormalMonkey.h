...
class CNormalMonkey final : public CMiniGame_Jino_Monster
{
private:
	explicit CNormalMonkey(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CNormalMonkey(const CNormalMonkey& rhs);
	virtual ~CNormalMonkey() = default;
...
private:
	HRESULT					Update_Move(_double fDeltaTime);
...
public:
	static CNormalMonkey*		Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, void* pArg = nullptr);
