...
HRESULT CFrustumMgr::SetUp_LightFrustumPlane()
{
	...
	_Matrix matVeiwInverse = XMMatrixInverse(nullptr, pPipeline->Get_Transform_Matrix(PLM_LIGHTVIEW));
	_Matrix matProjectInverse = XMMatrixInverse(nullptr, pPipeline->Get_Transform_Matrix(PLM_LIGHTPROJ));
	...
	////좌
	tLightFrustumPlane[0] = XMPlaneFromPoints(vWorldFrustumCubePos[0], vWorldFrustumCubePos[3], vWorldFrustumCubePos[4]);
	////우
	tLightFrustumPlane[1] = XMPlaneFromPoints(vWorldFrustumCubePos[2], vWorldFrustumCubePos[1], vWorldFrustumCubePos[6]);

	////상
	tLightFrustumPlane[2] = XMPlaneFromPoints(vWorldFrustumCubePos[1], vWorldFrustumCubePos[0], vWorldFrustumCubePos[5]);
	////하
	tLightFrustumPlane[3] = XMPlaneFromPoints(vWorldFrustumCubePos[3], vWorldFrustumCubePos[2], vWorldFrustumCubePos[7]);

	////앞
	tLightFrustumPlane[4] = XMPlaneFromPoints(vWorldFrustumCubePos[0], vWorldFrustumCubePos[1], vWorldFrustumCubePos[3]);
	////뒤
	tLightFrustumPlane[5] = XMPlaneFromPoints(vWorldFrustumCubePos[5], vWorldFrustumCubePos[4], vWorldFrustumCubePos[6]);
...
_bool CFrustumMgr::IsNeedToLightRender(_float3 vWorldPosition, _float fLenth)
{
	for (_uint i = 0; i < 6; i++)
	{
		if (tLightFrustumPlane[i].x * vWorldPosition.x + tLightFrustumPlane[i].y * vWorldPosition.y +
			tLightFrustumPlane[i].z * vWorldPosition.z + tLightFrustumPlane[i].w > fLenth)
			return false;
	}
